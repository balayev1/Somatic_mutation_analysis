...
dndscv = function(mutations, gene_list = NULL, refdb = "hg19", sm = "192r_3w", kc = "cgc81", cv = "hg19", max_muts_per_gene_per_sample = 3, max_coding_muts_per_sample = 3000, use_indel_sites = T, min_indels = 5, maxcovs = 20, constrain_wnon_wspl = T, outp = 3, numcode = 1, outmats = F, mingenecovs = 500) {
    ...
    message("Number of detected variants = ", nrow(mutations))
    ...
    # Add UTR columns to substitution model
    substmodel=cbind(substmodel, as.matrix(gsub("wspl","wutr5",as.matrix(substmodel[,"spl"])))); colnames(substmodel)[5]="utr5"
    substmodel=cbind(substmodel, as.matrix(gsub("wspl","wutr3",as.matrix(substmodel[,"spl"])))); colnames(substmodel)[6]="utr3"
    ...
    for (j in 1:length(RefCDS)) {
        ...
        if (!is.null(RefCDS[[j]]$utr5seq)) {
            RefCDS[[j]]$utr5seq = base::strsplit(as.character(RefCDS[[j]]$utr5seq), split="")[[1]]
            RefCDS[[j]]$utr5seq1up = base::strsplit(as.character(RefCDS[[j]]$utr5seq1up), split="")[[1]]
            RefCDS[[j]]$utr5seq1down = base::strsplit(as.character(RefCDS[[j]]$utr5seq1down), split="")[[1]]
        }
        if (!is.null(RefCDS[[j]]$utr3seq)) {
            RefCDS[[j]]$utr3seq = base::strsplit(as.character(RefCDS[[j]]$utr3seq), split="")[[1]]
            RefCDS[[j]]$utr3seq1up = base::strsplit(as.character(RefCDS[[j]]$utr3seq1up), split="")[[1]]
            RefCDS[[j]]$utr3seq1down = base::strsplit(as.character(RefCDS[[j]]$utr3seq1down), split="")[[1]]
        }
    }
    ...
    message("Number of mapped mutations = ", nrow(mutations))
    ...
    for (j in 1:length(RefCDS)) {
        RefCDS[[j]]$N = array(0, dim=c(192,6)) # Initialising the N matrices
    }
    ...
    for (j in 1:nrow(mutations)) {
        ...
        } else if (any(pos == RefCDS[[geneind]]$intervals_5utr)) { # Essential 5'UTR substitution
            impact[j] = "Essential_5UTR"; impind = 5
            pos_ind = (pos==RefCDS[[geneind]]$intervals_5utr)
            cdsnt = RefCDS[[geneind]]$utr5seq[pos_ind]
            ref3_cod[j] = sprintf("%s%s%s", RefCDS[[geneind]]$utr5seq1up[pos_ind], RefCDS[[geneind]]$utr5seq[pos_ind], RefCDS[[geneind]]$utr5seq1down[pos_ind])
            mut3_cod[j] = sprintf("%s%s%s", RefCDS[[geneind]]$utr5seq1up[pos_ind], mutations$mut_cod[j], RefCDS[[geneind]]$utr5seq1down[pos_ind])
            aachange[j] = ntchange[j] = codonsub[j] = "."
        
        } else if (any(pos == RefCDS[[geneind]]$intervals_3utr)) { # Essential 3'UTR substitution
            impact[j] = "Essential_3UTR"; impind = 6
            pos_ind = (pos==RefCDS[[geneind]]$intervals_3utr)
            cdsnt = RefCDS[[geneind]]$utr3seq[pos_ind]
            ref3_cod[j] = sprintf("%s%s%s", RefCDS[[geneind]]$utr3seq1up[pos_ind], RefCDS[[geneind]]$utr3seq[pos_ind], RefCDS[[geneind]]$utr3seq1down[pos_ind])
            mut3_cod[j] = sprintf("%s%s%s", RefCDS[[geneind]]$utr3seq1up[pos_ind], mutations$mut_cod[j], RefCDS[[geneind]]$utr3seq1down[pos_ind])
            aachange[j] = ntchange[j] = codonsub[j] = "."
    ...
    print(table(annot$impact)) # number of mutations of specific type and genomic region

    ## 3. Estimation of the global rate and selection parameters
    message("[3] Estimating global rates...")

    Lall = array(sapply(RefCDS, function(x) x$L), dim=c(192,6,length(RefCDS)))
    Nall = array(sapply(RefCDS, function(x) x$N), dim=c(192,6,length(RefCDS)))
    ...
    s1 = gsub("wmis","wall",gsub("wnon","wall",gsub("wspl","wall",gsub("wutr5","wall",gsub("wutr3","wall",substmodel)))))
    par1 = fit_substmodel(N, L, s1)$par # Substitution model with 1 selection parameter
    s2 = gsub("wnon","wtru",gsub("wspl","wtru",gsub("wutr5","wall",gsub("wutr3","wall",substmodel))))
    par2 = fit_substmodel(N, L, s2)$par # Substitution model with 2 selection parameter
    globaldnds = rbind(par, par1, par2)[c("wmis","wnon","wspl","wutr5","wutr3","wtru","wall"),]
    sel_loc = sel_cv = NULL

    ## 4. dNdSloc: variable rate dN/dS model (gene mutation rate inferred from synonymous subs in the gene only)

    genemuts = data.frame(gene_name = sapply(RefCDS, function(x) x$gene_name), n_syn=NA, n_mis=NA, n_non=NA, n_spl=NA, n_utr5=NA, n_utr3=NA, exp_syn=NA, exp_mis=NA, exp_non=NA, exp_spl=NA, exp_utr5=NA, exp_utr3=NA, stringsAsFactors=F)
    genemuts[,2:7] = t(sapply(RefCDS, function(x) colSums(x$N)))
    mutrates = sapply(substmodel[,1], function(x) prod(parmle[base::strsplit(x,split="\\*")[[1]]])) # Expected rate per available site
    genemuts[,8:13] = t(sapply(RefCDS, function(x) colSums(x$L*mutrates)))
    ...
    if (outp > 1) {
        ...
        selfun_loc = function(j) {
            ...
            # a. Neutral model: wmis==1, wnon==1, wspl==1, wutr5==1, wutr3==1
            mrfold = sum(y[1:6])/sum(y[5:10]) # Correction factor of "t" based on the obs/exp ratio of "neutral" mutations under the model
            ll0 = sum(dpois(x=x$N, lambda=x$L*mutrates*mrfold*t(array(c(1,1,1,1,1,1),dim=c(6,numrates))), log=T)) # loglik null model

            # b. Missense model: wmis==1, free wnon, free wspl, free wutr5, free wutr3
            mrfold = max(1e-10, sum(y[c(1,2)])/sum(y[c(7,8)])) # Correction factor of "t" based on the obs/exp ratio of "neutral" mutations under the model
            wfree = y[3:6]/y[9:12]/mrfold; wfree[y[3:6]==0] = 0
            llmis = sum(dpois(x=x$N, lambda=x$L*mutrates*mrfold*t(array(c(1,1,wfree),dim=c(6,numrates))), log=T)) # loglik free wmis

            # c. free wmis, wnon, wspl, wutr5 and wutr3
            mrfold = max(1e-10, y[1]/y[7]) # Correction factor of "t"
            w = y[2:6]/y[8:12]/mrfold; w[y[2:6]==0] = 0 # MLE of dN/dS based on the local rate (using syn muts as neutral)
            llall = sum(dpois(x=x$N, lambda=x$L*mutrates*mrfold*t(array(c(1,w),dim=c(6,numrates))), log=T)) # loglik free wmis, wnon, wspl
            w[w>1e4] = 1e4
        ...
        colnames(sel_loc) = c("wmis_loc","wnon_loc","wspl_loc", "wutr5_loc", "wutr3_loc", "pmis_loc","pall_loc")
        sel_loc$qmis_loc = p.adjust(sel_loc$pmis_loc, method="BH")
        sel_loc$qall_loc = p.adjust(sel_loc$pall_loc, method="BH")
        sel_loc = cbind(genemuts[,1:7],sel_loc)
    ...
    if (outp > 2) {
        ...
        selfun_cv = function(j) {
            ...
            # a. Neutral model
            indneut = 1:6 # vector of neutral mutation types under this model (1=synonymous, 2=missense, 3=nonsense, 4=essential_splice, 5=5'UTR, 6=3'UTR)
            opt_t = mle_tcv(n_neutral=sum(y[indneut]), exp_rel_neutral=sum(exp_rel[indneut]), shape=shape, scale=scale)
            mrfold = max(1e-10, opt_t/y[7]) # Correction factor of "t" based on the obs/exp ratio of "neutral" mutations under the model
            ll0 = sum(dpois(x=x$N, lambda=x$L*mutrates*mrfold*t(array(c(1,1,1,1,1,1),dim=c(6,numrates))), log=T)) + dgamma(opt_t, shape=shape, scale=scale, log=T) # loglik null model

            # b. Missense model: wmis==1, free wnon, free wspl, free wutr5, free wutr3
            indneut = 1:2
            opt_t = mle_tcv(n_neutral=sum(y[indneut]), exp_rel_neutral=sum(exp_rel[indneut]), shape=shape, scale=scale)
            mrfold = max(1e-10, opt_t/sum(y[7])) # Correction factor of "t" based on the obs/exp ratio of "neutral" mutations under the model
            wfree = y[3:6]/y[9:12]/mrfold; wfree[y[3:6]==0] = 0
            llmis = sum(dpois(x=x$N, lambda=x$L*mutrates*mrfold*t(array(c(1,1,wfree),dim=c(6,numrates))), log=T)) + dgamma(opt_t, shape=shape, scale=scale, log=T) # loglik free wmis

            # c. Truncating muts model: free wmis, free wutr5, free wutr3, wnon==wspl==1
            indneut = c(1,3,4)
            opt_t = mle_tcv(n_neutral=sum(y[indneut]), exp_rel_neutral=sum(exp_rel[indneut]), shape=shape, scale=scale)
            mrfold = max(1e-10, opt_t/sum(y[7])) # Correction factor of "t" based on the obs/exp ratio of "neutral" mutations under the model
            wfree = y[c(2,5,6)]/y[c(8,11,12)]/mrfold; wfree[y[c(2,5,6)]==0] = 0
            lltrunc = sum(dpois(x=x$N, lambda=x$L*mutrates*mrfold*t(array(c(1,wfree[1],1,1, wfree[2:length(wfree)]),dim=c(6,numrates))), log=T)) + dgamma(opt_t, shape=shape, scale=scale, log=T) # loglik free wmis

            # d. Free selection model: free wmis, free wnon, free wspl, free wutr5, free wutr3
            indneut = 1
            opt_t = mle_tcv(n_neutral=sum(y[indneut]), exp_rel_neutral=sum(exp_rel[indneut]), shape=shape, scale=scale)
            mrfold = max(1e-10, opt_t/sum(y[7])) # Correction factor of "t" based on the obs/exp ratio of "neutral" mutations under the model
            wfree = y[2:6]/y[8:12]/mrfold; wfree[y[2:6]==0] = 0
            llall_unc = sum(dpois(x=x$N, lambda=x$L*mutrates*mrfold*t(array(c(1,wfree),dim=c(6,numrates))), log=T)) + dgamma(opt_t, shape=shape, scale=scale, log=T) # loglik free wmis
            ...
            if (constrain_wnon_wspl == 0) {
                ...
                return(c(wfree,p))
se
            } else { # d2. Free selection model: free wmis, free wnon==wspl, free wutr5, free wutr3

                wmisfree = y[2]/y[6]/mrfold; wmisfree[y[2]==0] = 0
                wutr5free = y[5]/y[11]/mrfold; wmisfree[y[5]==0] = 0
                wutr3free = y[6]/y[12]/mrfold; wmisfree[y[6]==0] = 0
                wtruncfree = sum(y[3:4])/sum(y[7:8])/mrfold; wtruncfree[sum(y[3:4])==0] = 0
                llall = sum(dpois(x=x$N, lambda=x$L*mutrates*mrfold*t(array(c(1,wmisfree,wtruncfree,wtruncfree,wutr5free,wutr3free),dim=c(6,numrates))), log=T)) + dgamma(opt_t, shape=shape, scale=scale, log=T) # loglik free wmis, free wnon==wspl
                p = 1-pchisq(2*c(llall_unc-llmis,llall-c(lltrunc,ll0)),df=c(1,1,2))
                return(c(wmisfree,wtruncfree,wtruncfree,wutr5free,wutr3free, p))
            }
        }

        sel_cv = as.data.frame(t(sapply(1:nrow(genemuts), selfun_cv)))
        colnames(sel_cv) = c("wmis_cv","wnon_cv","wspl_cv","wutr5_cv","wutr3_cv", "pmis_cv","ptrunc_cv","pallsubs_cv")
        ...
        sel_cv = cbind(genemuts[,1:7],sel_cv)
