...
def genome_index(filename,GD):	
    ...
    for index in range(len(genome)):
        if ">" in genome[index][0]:
            position_list.append(index)
    chr_dict={}
    for index in range(len(position_list)):
        if index!=len(position_list)-1:
            list_of_lists=genome[position_list[index]+1:position_list[index+1]]
            flattened_list = [y for x in list_of_lists for y in x]
            chr_dict[genome[position_list[index]][0][1:]]=''.join(
                    flattened_list)
        if index==len(position_list)-1:
            list_of_lists=genome[position_list[index]+1:]
            flattened_list = [y for x in list_of_lists for y in x]
            chr_dict[genome[position_list[index]][0][1:]]=''.join(
                    flattened_list)
    return chr_dict
...
def enrichment(WD, output, motifs=['TCW_WGA','CC_GG','WRC_GYW','WA_TW','AG/CT-GG/CC']):
    """
    Produces 2 files: 
    ...
    motifs: list
        ...
        Default=['TCW_WGA','CC_GG','WRC_GYW','WA_TW','AG/CT-GG/CC']
    ...
    if 'TCW_WGA' in motifs:
        ...
        for num, file in enumerate(wd, start=0):
            ...
            for index in range(len(new)):
                if new[index][0]=="#CHROM":
                    _=apobec3a_b.write('\t'.join(new[index])+'\n'); new=new[index+1:]; break
                _=apobec3a_b.write(' '.join(new[index])+('\n'))
